...
| the archive of a merged branch: its whole `.revali/<branch>/` (state, reviews, `tests.md`, logs) | `revali merge` | you; nothing in revali reads it | `~/.revali/archive/<owner>__<repo>/<pr>-<branch>/` (a taken name gets a `-<YYYYMMDD-HHMMSS>` suffix) | `[paths] archive_dir`; relative = under `~/.revali/`, `""` = delete instead |
