...
def render_section_summary(o: ValidationOutcome, state_dir: str) -> str:
    ...
    if o.report:
        out += ["| step | exit |", "|---|---|"]
        for s in o.report.steps:
            out.append("| %s | %s%s |" % (s.name, s.returncode, " (timed out)" if s.timed_out else ""))
        out.append("")
    if o.result == FAIL:
        out.append("failed at step `%s`" % o.failed_step)
        if o.diagnosis:
            out.append("diagnosis (%s): cause **%s**, %d failure(s) examined"
                       % (o.model_actual or "?", o.diagnosis.get("cause"), len(o.diagnosis.get("failures", []))))
        elif o.diagnosis_error:
