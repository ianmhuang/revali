...
def ensure_pr(ctx: Context, state: State, rdir: str, log: Optional[RunLog]) -> None:
    ...
    if pr is None:
        closed = [p for p in gitops.gh_pr_any(ctx.branch, root, _log(log)) if str(p.get("state", "")).upper() != "OPEN"]
        if closed:
            raise Stop(EXIT_ERROR, "PR #%s for this branch is %s; reopen it or start a new branch"
...
def pr_body(ctx: Context, state: Optional[State]) -> str:
    ...
    if state and state.rounds:
        ...
        for r in state.rounds:
            rows.append("| %d | %s | %s%s | $%.2f |" % (r.get("round", 0), r.get("verdict", "?"), r.get("model", "?"),
                                                        " (fallback)" if r.get("fallback") else "", r.get("cost_usd", 0.0)))
        rows.append("")
...
def post_comment(ctx: Context, state: State, rdir: str, name: str, body: str, log: Optional[RunLog]) -> bool:
    ...
    state.save(rdir)
    res = run_retry(resolve("gh") + ["pr", "comment", str(state.pr_number), "--body-file", path],
                    cwd=ctx.repo_root, log=_log(log), timeout=120)
    state.pending_effect = ""
    state.save(rdir)
    if not res.ok:
        if log:
            log.stage("pr", "warning: could not post comment %s: %s" % (name, res.text.strip()[:200]))
        return False
    return True
