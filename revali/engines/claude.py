...
def failure_message(
    role: str, payload: dict, returncode: int, budget_usd: float, raw_path: str
) -> str:
    ...
    if (
        ...
        return (
            "%s ran out of budget ($%.2f) after %s turns, spent $%.2f; raise budget_usd for this "
    ...
    return "%s session failed (exit %d): %s; raw output saved to %s" % (
        role,
        returncode,
        detail,
        raw_path,
...
def permission_args(request: EngineRequest) -> List[str]:
    ...
    if request.shell_allow:
        args += [
            "--allowedTools",
            " ".join("Bash(%s *)" % prefix for prefix in request.shell_allow),
        ]
    return args
...
class ClaudeEngine(Engine):
    ...
    def run(self, request: EngineRequest, log: Optional[RunLog] = None) -> EngineResult:
        ...
        try:
            ...
            raise Stop(
                EXIT_ERROR,
                "%s session timed out after %d minutes" % (role, request.timeout_s // 60),
            ) from exc
        write_text(
            request.raw_path,
        ...
        try:
            ...
            raise Stop(
                EXIT_ERROR,
                "%s returned invalid JSON (exit %d): %s; raw output saved to %s"
                % (role, res.returncode, text, request.raw_path),
            ) from exc
        if not isinstance(payload, dict):
            raise Stop(
                EXIT_ERROR, "%s output is not a JSON object; saved to %s" % (role, request.raw_path)
            )
