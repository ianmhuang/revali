...
class ReadmeAgents(unittest.TestCase):
    ...
    def test_several_agents_section_content(self):
        ...
        for phrase in (
            "git worktree add ../<name> -b <branch>",  # one worktree per agent
    ...
    def test_merge_bullet_lists_the_worktree_side_effects(self):
        body = unwrap(
            read("docs/side-effects.md").split("# What revali does to your repository\n", 1)[1]
        )
...
class SshStubRmdir(unittest.TestCase):
    ...
    def stub(self, command):
        ...
        return subprocess.run(
            [sys.executable, SSH_STUB, "box", command], env=env, capture_output=True, text=True
        )
    ...
    def test_every_argument_is_removed_leaf_first(self):
        ...
        res = self.stub(
            ...
            ".revali/sandbox/sample/feature__mul .revali/sandbox/sample"
        )
        self.assertEqual(res.returncode, 0, res.stderr)
        self.assertFalse(
            os.path.isdir(os.path.join(self.remote, ".revali", "sandbox", "sample", "feature__mul"))
        )
        self.assertFalse(os.path.isdir(os.path.join(self.remote, ".revali", "sandbox", "sample")))
        self.assertTrue(
            os.path.isdir(os.path.join(self.remote, ".revali", "sandbox"))
        )  # not named: stays
...
class SshHostCleanup(SshCase):
    """AC-6 end to end: after a scoped run over the ssh stubs, <repo>/<branch> and <repo> are
    gone."""
    ...
    def test_branch_and_repo_directories_are_gone(self):
        ...
        self.assertFalse(
            os.path.exists(os.path.join(sandbox, "sample", "feature__mul", "validate-r1"))
        )
...
class SandboxCloneFailure(RepoCase):
    ...
    def test_failed_checkout_leaves_nothing_on_the_host(self):
        ...
        self.assertFalse(
            os.path.exists(os.path.join(self.sand, "sample", "feature__mul", "validate-r1"))
        )
    ...
    def test_failed_clone_leaves_nothing_on_the_host(self):
        ...
        with self.assertRaises(RunnerError):
            self.make_runner().run(
                missing, head, [("test", "true")], {}, logs, "validate-r1", scope="feature__mul"
            )
