...
class StopClosesTheRun(RepoCase):
    ...
    def _dead_state(self, **fields):
        st = State(
            ...
            cost_usd=1.5,
            fixes=1,
            rounds=[{"round": 1, "verdict": "CHANGES_REQUESTED"}],
            **fields,
        )
        st.save(self.rdir())
        return st
    ...
    def test_dead_path_write_failure_is_one_error_line(self):
        ...
        with mock.patch.object(
            State, "set_stage", side_effect=PermissionError("state.json is busy")
        ):
    ...
    def test_live_path_appends_one_row(self):
        ...
        rows = self._history()
        self.assertEqual(
            [(r["stage"], r["exit"], r["branch"]) for r in rows],
...
class StatsWithStoppedRuns(unittest.TestCase):  # AC-4
    ...
    def test_a_stop_on_another_branch_is_its_own_pipeline(self):
        text = summarise(
            [self._row("merged", 0), self._row("stopped", 1, branch="feature/div", pr=8)]
        )
