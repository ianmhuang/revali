...
from tests.helpers import TEST_REVIEW_MUL, RepoCase, claude_entry, git, run_cli
...
def cli_result(**kw):
    ...
    payload = {
        ...
        "result": "",
        "errors": [],
        "modelUsage": {"claude-fable-5": {"costUSD": 0.1}},
        "permission_denials": [],
    }
    payload.update(kw)
...
class AC4FailureIsNamed(RepoCase):
    ...
    def test_other_cli_error_quotes_its_text(self):
        ...
        self.assertEqual(code, EXIT_ERROR, out)
        self.assertIn(
            "reviewer session failed (exit 1): Invalid API key. Fix external API key", out
        )
    ...
    def test_plain_text_cli_error_is_quoted(self):
        ...
        self.assertEqual(code, EXIT_ERROR, out)
        self.assertIn(
            "reviewer returned invalid JSON (exit 2): error: unknown option '--json-schema'", out
        )
    ...
    def test_diagnoser_budget_exhaustion_is_named_in_tests_md(self):
        ...
        self.claude(
            claude_entry(), {"exit": 1, "raw_stdout": budget_exhausted(spent=0.61, turns=4)}
        )
        code, out = run_cli(["run", "--foreground"])
...
class AC5TreeLeftClean(RepoCase):
    ...
    def test_successful_session_keeps_its_tests(self):
        ...
        self.assertNotIn("unfinished test file", out)
        self.assertIn(
            "tests/test_review_mul.py", git(["show", "--stat", "--format=", "HEAD"], self.repo)
        )
