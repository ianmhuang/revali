...
class ApprovePath(RepoCase):
    def test_full_round(self):
        ...
        self.assertEqual(state.last_verdict, "APPROVE")
        ...
        # runner: smoke run with the new file
        rn = self.fake_calls("runner")
        self.assertEqual(len(rn), 1)
        self.assertEqual(rn[0]["extra_files"], ["tests/test_review_mul.py"])
        self.assertEqual(rn[0]["steps"], ["new_test"])
        # history
        rows = read_history(os.path.join(self.home, "history.jsonl"))
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["last_verdict"], "APPROVE")
