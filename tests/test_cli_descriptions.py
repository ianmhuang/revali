...
class SubcommandHelp(unittest.TestCase):
    ...
    def test_one_helper_registers_every_command(self):                                     # AC-3
        ...
        helper = inspect.getsource(cli._command)
        self.assertIn("help=sentence", helper)
        self.assertIn("description=sentence", helper)
        self.assertEqual(inspect.getsource(cli.build_parser).count("_command(sub, "), len(COMMANDS))
    ...
    def test_dispatch_is_unchanged(self):                                                  # AC-5
        ...
        self.assertEqual((args.verbose, args.foreground, args.dry_run, args.base), (True, True, True, "dev"))
...
class CloseStoppedRestoresTimestamps(unittest.TestCase):                                   # AC-4
    def test_started_and_updated_at_survive_a_failed_write(self):
        ...
        state = State(branch="b", base="main", stage="review", message="reviewer round 1", last_exit=-1,
                      started_at="2026-01-01T00:00:00+0000", updated_at="2026-01-01T00:05:00+0000")
