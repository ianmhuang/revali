...
class RefusalBeforeTheCiWait(MergeCase):
    ...
    def test_primary_tree_is_refused_without_polling_ci(self):
        ...
        self.assertEqual(self.gh_calls("pr", "checks"), [], "the CI wait ran before the refusal:\n" + out)
...
class DetachedStopFollowsTheDeadRecord(RepoCase):
    ...
    def dead_run(self, stage="validate"):
        State(repo="owner/repo", branch="feature/mul", base="main", stage=stage,
              message="validation 1", last_exit=-1, pr_number=7).save(self.rdir())
