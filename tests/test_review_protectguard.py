...
class RestoreAndBounce(ProtectedCase):
    def test_tampered_files_restored_and_session_sent_back_once(self):
        ...
        self.assertEqual(files_in_commit(state.test_commits[0], self.repo), ["tests/test_review_mul.py"])
    ...
    def test_bounce_shares_the_single_retry_with_ac_gaps(self):
        partial = approve_response(tests=[{"path": "tests/test_review_mul.py", "purpose": "p",
                                           "covers": ["AC-1"], "expected": "e"}])
...
class DeletionRestore(ProtectedCase):
    def test_deleted_tracked_files_are_restored_and_new_files_kept(self):
        ...
        self.assertEqual(sorted(restored), ["tests/__init__.py", "tests/test_review_old.py"])   # AC-1
        self.assertEqual(self.read("tests/test_review_old.py"), OLD_PR_TESTS)
...
class OwnFilesAccepted(ProtectedCase):
    def test_round_two_updates_its_own_file_and_adds_another(self):
        ...
        ok = approve_response(
            ...
            tests=[{"path": "tests/test_review_mul.py", "purpose": "p", "covers": ["AC-1"], "expected": "e"},
                   {"path": "tests/test_review_extra.py", "purpose": "p", "covers": ["AC-2"], "expected": "e"}])
        ...
        self.assertEqual(files_in_commit(state.test_commits[1], self.repo),
        ...
        self.assertEqual(sorted(state.test_files), ["tests/test_review_extra.py", "tests/test_review_mul.py"])
        self.assertEqual(self.read("tests/test_review_old.py"), OLD_PR_TESTS)
