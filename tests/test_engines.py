...
CFG = EngineCfg(
    name="claude", tiers=["haiku", "sonnet", "opus", "fable"], helper_prefix="claude-haiku"
)
...
class PermissionMappingTests(unittest.TestCase):
    def test_reviewer_permissions(self):
        args = permission_args(
            request(may_write=["tests"], shell_allow=["git diff", "git log", "git show"])
        )
        self.assertEqual(
            args,
            [
    ...
    def test_read_only_wins(self):
        args = permission_args(
            request(read_only=True, may_write=["tests"], shell_allow=["git diff"])
        )
...
class FailureMessageTests(unittest.TestCase):
    ...
    def test_other_errors_are_quoted(self):
        ...
        self.assertIn(
            "reviewer session failed (exit 1): boom",
            failure_message("reviewer", payload, 1, 2.0, "r"),
        )
        self.assertIn(
            "no error text", failure_message("diagnoser", {"is_error": True}, 1, 1.0, "r")
        )
        self.assertIn(
            "no error text",
            failure_message("diagnoser", {"is_error": True, "result": None}, 1, 1.0, "r"),
        )
...
class ModelFamilyTests(unittest.TestCase):
    ...
    def test_helper_prefix_from_config(self):
        engine = ClaudeEngine(
            EngineCfg(name="claude", tiers=["opus", "fable"], helper_prefix="claude-sonnet")
        )
