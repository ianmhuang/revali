...
from revali import EXIT_ERROR, EXIT_OK, gitops, procs
...
class DetachedStop(RepoCase):
    ...
    def test_stop_finds_the_trees_run_through_tree_lock(self):
        ...
        self.assertEqual(
            gitops.current_branch(self.repo), "HEAD"
        )  # stop does not touch the checkout
...
class TaskkillWindow(unittest.TestCase):
    ...
    def test_dead_pid_spawns_nothing(self):
        with (
            mock.patch("os.name", "nt"),
            mock.patch.object(procs, "pid_alive", lambda pid: False),
...
class ReviewJsonHead(RepoCase):
    ...
    def test_review_json_has_head_sha_next_to_commit(self):
        ...
        self.assertEqual(
            data["commit"], gitops.rev_parse("HEAD", self.repo)
        )  # the test commit on top
