...
class RestoreFromHead(RepoCase):
    def test_staged_modification_of_a_protected_test_is_undone_in_index_and_tree(self):
        ...
        self.assertEqual(
            git(["diff", "--cached", "--name-only"], self.repo).strip(), ""
        )  # AC-1: index
...
class PorcelainPaths(RepoCase):
    def test_untracked_and_modified_paths_arrive_unquoted(self):
        ...
        self.assertEqual(
            sorted(dirty_paths(self.repo, (".revali/",))),
            sorted(
                [
                    "M " + TRACKED_SPACED,
                    "M " + TRACKED_NON_ASCII,
    ...
    def test_guards_restore_odd_names(self):
        ...
        self.assertEqual(
            sorted(restore_protected_tests(ctx, State(), None)),
...
class PipelineWithOddNames(RepoCase):
    def test_tracked_spaced_file_restored_and_new_odd_files_committed(self):
        ...
        new_files = {
            NEW_SPACED: TEST_REVIEW_MUL,
            NEW_NON_ASCII: TEST_REVIEW_MUL.replace("MulTests", "T2"),
        }
        answer = approve_response(
            tests=[
                {"path": p, "purpose": "p", "covers": ["AC-1", "AC-2"], "expected": "e"}
                for p in new_files
            ]
        )
        first = claude_entry(answer, write_tests=False)
        first["write_files"] = dict(
            new_files, **{TRACKED_SPACED: HOLLOW, TRACKED_NON_ASCII: HOLLOW}
        )
        ...
        self.assertEqual(
            files_in_commit(state.test_commits[0], self.repo), sorted(new_files)
        )  # AC-3: committed
