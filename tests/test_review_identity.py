...
from revali import EXIT_ACTION, EXIT_ERROR, EXIT_OK, NAME, VERSION, gitops
...
class IdentityCase(RepoCase):
    ...
    def state(self, stage, message, last_exit):
        State(
            branch="feature/mul", base="main", stage=stage, message=message, last_exit=last_exit
        ).save(self.rdir())
...
class WaitOpensWithTheIdentityLine(IdentityCase):
    ...
    def test_still_running_keeps_the_pid_and_exit_4(self):  # AC-2, AC-3, AC-4
        ...
        self.assertEqual(lines[0], self.identity())
        self.assertTrue(
            lines[1].startswith("still running (pid %d), stage starting" % os.getpid()), out
        )
    ...
    def test_died_keeps_the_pid_from_the_stale_lock(self):  # AC-2, AC-3, AC-4
        ...
        self.assertTrue(
            lines[1].startswith(
                "error: the run (pid %d) died at stage 'review' without a result" % DEAD_PID
            ),
            out,
        )
    ...
    def test_died_without_a_lock_has_no_pid_to_show(self):  # AC-4
        ...
        self.assertEqual(lines[0], self.identity())
        self.assertTrue(
            lines[1].startswith("error: the run died at stage 'validate' without a result"), out
        )
