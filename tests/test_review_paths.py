...
class StateAndLogsDir(RepoCase):
    ...
    def test_every_command_uses_the_configured_directories(self):
        ...
        for rel in (
            ...
            "state.json",
            "trace/revali.log",
            "trace/review-r1-1.raw.json",
            "trace/prompt-r1-1.md",
            "trace/pr-body.md",
            "trace/comment-review-1.md",
        ):
    ...
    def test_broken_config_still_uses_the_configured_state_dir(self):
        ...
        self.write(
            "revali.toml",
            self.read("revali.toml").replace("budget_usd = 1.0", 'budget_usd = "lots"'),
        )
...
class SandboxDir(RepoCase):
    ...
    def script_for(self, sandbox_dir, label):
        r = WslRunner(
            PlatformCfg(
                runner="wsl", distro="Ubuntu", command_timeout_min=1, sandbox_dir=sandbox_dir
            )
        )
...
class FileOverrides(RepoCase):
    def test_project_files_replace_the_shipped_ones(self):
        ...
        self.assertTrue(
            reviewer["prompt"].startswith("REVIEW PROMPT feature/mul round 1"),
            reviewer["prompt"][:80],
        )
        ...
        self.assertTrue(
            diagnoser["prompt"].startswith("DIAG PROMPT new_test"), diagnoser["prompt"][:80]
        )
