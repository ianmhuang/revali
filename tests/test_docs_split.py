...
class DocsHoldTheSections(unittest.TestCase):
    ...
    def test_workflow(self):
        ...
        for phrase in (
            "`repo: <working tree root>  branch: <branch>`",
            "`.revali/tree.lock`",
            "After exit code 2",
            "died at stage",
            "skips the reviewer and goes to validation",
        ):
...
class RefusalBeforeTheCiWait(ReadyRepo):
    ...
    def test_primary_tree_is_refused_before_wait_for_checks(self):
        ...
        with mock.patch.object(
            merge, "wait_for_checks", side_effect=AssertionError("CI wait must not run")
        ):
        ...
        line = next(line for line in out.splitlines() if "ERROR: main is checked out in" in line)
...
class BranchDeleteFailure(ReadyRepo):
    ...
    def test_worktree_mode_reports_a_kept_branch(self):
        ...
        self.assertEqual(code, EXIT_OK, out)
        self.assertIn(
            "detached at the merged main, local branch feature/mul kept (git branch -D failed: "
            "error: fake refusal",
            out,
        )
...
class BranchLockRace(ReadyRepo):
    ...
    def test_lock_held_between_check_and_acquire_is_an_error_line(self):
        ...
        with (
            mock.patch.object(pipeline, "acquire_lock", held),
...
class WorktreeModeCondition(unittest.TestCase):
    ...
    def test_several_agents_states_when_worktree_mode_applies(self):
        body = " ".join(
            h2_section(read("docs", "workflow.md"), "Several agents on one repository").split()
        )
