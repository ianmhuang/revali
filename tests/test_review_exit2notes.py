...
def finding(fid, sev, kind, line):
    return {"id": fid, "file": "src/calc.py", "line": line, "severity": sev, "kind": kind,
            "text": "see line %d" % line, "suggestion": ""}
...
DIAGNOSIS = {"summary": "mul returns the wrong product.", "cause": "code",
...
def line_with(text, token):
    return next((l for l in text.splitlines() if token in l), "")


class ChangesRequested(RepoCase):
    def test_non_blocking_findings_listed_once_each(self):
        self.claude(claude_entry(approve_response(verdict="CHANGES_REQUESTED", findings=[F1, F2, F3, F4])))
        code, out = run_cli(["run", "--foreground"])
        self.assertEqual(code, EXIT_ACTION, out)
        msg = action_message(out)
        self.assertIn("3 non-blocking", msg)                                         # AC-5: count
        for fid, sev, kind, line in (("F2", "low", "correctness", 5), ("F3", "medium", "convention", 8),
                                     ("F4", "low", "security", 11)):
...
class NeedsInfo(RepoCase):
    def test_questions_then_non_blocking_findings(self):
        data = approve_response(verdict="NEEDS_INFO", questions=["Are floats in scope?"], tests=[],
                                findings=[F3, F2])
...
class ValidationFail(RepoCase):
    def test_findings_of_the_approving_round_not_of_the_earlier_one(self):
        self.runner_scenario({"default": 0, "results": {"validate-r2": {"new_test": 1}},
        ...
        self.claude(claude_entry(ok, write_tests=False),
...
class OtherExitsUnchanged(RepoCase):
    ...
    def test_pipeline_error(self):
        ...
        self.assertNotIn("non-blocking", out)
        self.assertEqual(State.load(self.rdir()).message,
