...
from revali import EXIT_ERROR, EXIT_OK, gitops, merge
...
class SandboxLayoutTests(unittest.TestCase):
    ...
    def test_scoped_and_unscoped_paths(self):
        self.assertEqual(
            sandbox_dir("$HOME/.revali/sandbox", "sample", "feature__mul", "validate-r1"),
            "$HOME/.revali/sandbox/sample/feature__mul/validate-r1",
        )
        self.assertEqual(
            sandbox_dir("$HOME/.revali/sandbox", "sample", "", "validate-r1"),
            "$HOME/.revali/sandbox/sample/validate-r1",
        )  # direct callers keep the old layout
    ...
    def test_wsl_runner_clones_under_the_branch(self):
        ...
        with (
            mock.patch.object(WslRunner, "wslpath", lambda self, p: p),
...
class SshLayoutTests(SshCase):
    ...
    def test_staging_dirs_sit_under_the_branch(self):
        ...
        calls = self.calls()
        self.assertTrue(
            calls[1][1][-1].startswith("box:.revali/sandbox/sample/feature__mul/validate-r1-in"),
            calls[1][1],
        )
        self.assertEqual(
        ...
        # the <branch> and <repo> directories are gone too (PR #23 F6: the stub now removes every
        # rmdir argument)
...
class MergeLockTests(RepoCase):
    ...
    def test_merge_holds_the_tree_lock_and_a_run_is_refused_meanwhile(self):
        ...
        with (
            mock.patch.object(merge, "do_merge", fake_merge),
        ...
        self.assertIn(
            "already in progress in this working tree on branch feature/mul", seen["run"][1]
        )
        self.assertFalse(os.path.isfile(self.tree_lock()))
        self.assertFalse(os.path.isfile(lock_path(self.rdir())))
    ...
    def test_merge_refused_while_a_run_holds_the_tree(self):
        ...
        os.makedirs(os.path.dirname(self.tree_lock()), exist_ok=True)
        write_json_atomic(
            self.tree_lock(), {"pid": child.pid, "branch": "feature/other", "since": "x"}
        )
...
class WorktreeMergeTests(RepoCase):
    ...
    def setUp(self):
        ...
        def drop():
            os.chdir(self.repo)
            subprocess.run(
                ["git", "worktree", "remove", "--force", self.wt],
                cwd=self.repo,
                capture_output=True,
            )
    ...
    def ready(self):
        State(
            ...
            last_exit=EXIT_OK,
            pr_number=7,
            head_sha=gitops.rev_parse("HEAD", self.wt),
            test_files=["tests/test_review_mul.py"],
        ).save(self.rdir())
        git(
            ["push", "-q", "-u", "origin", "feature/mul"], self.wt
        )  # what the pr stage does in a real run

    def remote_heads(self):
        return sorted(
            line.split("refs/heads/")[1]
            for line in git(["ls-remote", "--heads", "origin"], self.wt).splitlines()
        )

    def test_worktree_holding(self):
        self.assertEqual(
            os.path.normpath(gitops.worktree_holding("main", self.wt)), os.path.normpath(self.repo)
        )
