...
class DefaultsFileTests(unittest.TestCase):
    def test_defaults_cover_every_field(self):
        ...
        for name in plat.__dataclass_fields__:
            if name != "name":
                self.assertIn(
                    name, d["validate"]["platform"], "validate.platform.%s has no default" % name
                )
...
class LayeringTests(unittest.TestCase):
    def test_user_overrides_default_project_overrides_user(self):
        user = {
            "review": {"model": "opus", "budget_usd": 0.5},
            "merge": {"method": "rebase"},
        ...
        project = MINIMAL.replace(
            'new_test = "pytest tests"', 'new_test = "pytest tests"\ndistro = "Debian"'
        )
    ...
    def test_unknown_key_in_user_layer(self):
        ...
        self.assertTrue(
            any("user config: review: unknown key 'modle'" in p for p in cm.exception.problems)
        )

    def test_merge_layers_platform_defaults(self):
        merged = merge_layers(
            {"validate": {"platform": {"runner": "wsl", "distro": "U"}}},
            {"validate": {"linux": {"distro": "V"}}},
            {"validate": {"linux": {"setup": "s"}, "other": {}}},
        )
        self.assertEqual(
            merged["_platforms"]["linux"], {"runner": "wsl", "distro": "V", "setup": "s"}
        )
        ...
        self.assertEqual(
            later["_platforms"]["linux"], {"runner": "wsl", "distro": "W", "setup": "s"}
        )
...
class EngineAndPathChecks(unittest.TestCase):
    def test_old_engine_value_gets_migration_message(self):
        ...
        self.assertTrue(
            any(
                "now review.strategy" in p and 'engine = "claude"' in p
    ...
    def test_unknown_engine_lists_available(self):
        ...
        self.assertTrue(
            any(
                "validate.engine 'codex' is unknown (available: claude)" in p
                for p in cm.exception.problems
            )
        )
