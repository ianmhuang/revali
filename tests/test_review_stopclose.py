...
class StopCloseCase(RepoCase):
    def dead_state(self, stage="review", **fields):
        base = dict(repo="owner/repo", branch="feature/mul", base="main", stage=stage,
                    message="reviewer round 1", last_exit=-1, pr_number=7, fixes=3, cost_usd=2.5,
                    rounds=[{"round": 1, "verdict": "CHANGES_REQUESTED"}, {"round": 2, "verdict": "APPROVE"}],
                    last_verdict="APPROVE", models_used=["claude-fable-5"])
        base.update(fields)
...
class AFailedStateWriteIsContained(StopCloseCase):
    def test_dead_path_prints_one_error_line_and_returns_1(self):
        ...
        self.assertTrue("wait" in error_lines[0] and "status" in error_lines[0], error_lines[0])   # AC-2: what they show
...
class EveryStopThatMarksStoppedWritesOneRow(StopCloseCase):
    ...
    def test_a_stop_that_changes_nothing_appends_no_row(self):
        ...
        for stage, exit_code in (("needs_action", EXIT_ACTION), ("ready_to_merge", EXIT_OK),
                                 ("error", EXIT_ERROR), ("stopped", EXIT_ERROR)):
...
class StatsCountsTheStoppedEpisode(StopCloseCase):
    ...
    def test_a_later_row_for_the_same_pipeline_supersedes_the_stop(self):
        ...
        append_history(history_path(), {
            "repo": "owner/repo", "branch": "feature/mul", "base": "main", "stage": "merged",
            "exit": EXIT_OK, "rounds": 3, "fixes": 3, "last_verdict": "PASS", "cost_usd": 4.0,
            "models": ["claude-fable-5"], "fallback": False, "pr": 7})
        code, out = run_cli(["stats"])
        self.assertEqual(code, EXIT_OK, out)
    ...
    def test_a_stop_on_another_pr_is_its_own_pipeline(self):
        append_history(history_path(), {
            "repo": "owner/repo", "branch": "feature/div", "base": "main", "stage": "merged",
            "exit": EXIT_OK, "rounds": 1, "fixes": 0, "last_verdict": "PASS", "cost_usd": 1.0,
            "models": ["claude-fable-5"], "fallback": False, "pr": 8})
        self.dead_state(stage="review")
        run_cli(["stop"])
        code, out = run_cli(["stats"])
