...
that is unreachable stops the run with exit 1 naming the host; a developer machine
without scp stops the same way (round 1, F2); the probe asks the host for git, timeout
...
class SshProbe(RepoCase):
    ...
    def test_missing_scp_stops_with_exit_1_in_preflight(self):
        # round 1, F2: the transport needs scp as well as ssh; a machine without it must
        # stop in preflight, not with a traceback after the PR is opened
