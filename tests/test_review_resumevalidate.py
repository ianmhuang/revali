...
class ResumeCase(RepoCase):
    def approve_then_die_before_validation(self):
        ...
        self.assertEqual(
            state.head_sha, state.test_commits[0]
        )  # HEAD is the reviewer's test commit
        return state
    ...
    def rerun(self, *claude_entries):
        ...
        self.scenario(
            {
                "prs_open": [
                    {"number": 7, "url": PR_URL, "isDraft": True, "title": "Add mul to calc"}
                ]
            }
        )
    ...
    def gh(self, *prefix):
        return [
            c["argv"] for c in self.fake_calls("gh") if c["argv"][: len(prefix)] == list(prefix)
        ]

    def comment_names(self):
        return [
            os.path.basename(argv[argv.index("--body-file") + 1])
            for argv in self.gh("pr", "comment")
        ]
...
class ResumeAtValidation(ResumeCase):
    def test_the_rerun_validates_the_approved_round_without_a_new_review(self):
        ...
        self.assertEqual(
            [r["label"] for r in self.fake_calls("runner")],
        ...
        self.assertEqual(
            self.comment_names(), ["comment-review-1.md", "comment-validate-1.md"]
        )  # AC-4: no new review comment
...
class NoResume(ResumeCase):
    def test_a_new_head_gets_a_new_round(self):
        ...
        self.assertEqual(
            self.comment_names(),
            ["comment-review-1.md", "comment-review-2.md", "comment-validate-1.md"],
        )
    ...
    def test_a_round_that_already_has_a_validation_is_not_resumed_again(self):
        ...
        # same HEAD, round 1 validated: another run is a new round, not a third validation of round
        # 1
        self.claude(claude_entry())
        code, out = run_cli(["run", "--foreground"])
        self.assertEqual(code, EXIT_OK, out)
...
class Documented(unittest.TestCase):
    ...
    def test_readme_defaults_and_template_carry_the_change(self):
        ...
        self.assertIn(
            "validation", readme.split("without a result", 1)[1][:1500]
        )  # AC-6: the resume, nearby
