...
class EveryArgumentHasAHelpText(unittest.TestCase):
    ...
    def _arguments(self, name):
        return [
            a for a in self.sub.choices[name]._actions if not isinstance(a, argparse._HelpAction)
        ]
    ...
    def test_the_argument_set_is_unchanged(self):
        for name in COMMANDS:
            with self.subTest(command=name):
                self.assertEqual(
                    {a.dest for a in self._arguments(name)}, self.EXPECTED[name]
                )  # AC-5: no new flags
        ...
        for name in COMMANDS:
            ...
            for action in self._arguments(name):
                with self.subTest(command=name, argument=action.dest):
                    self.assertTrue(
                        action.help and action.help.strip(), "%s %s" % (name, action.dest)
                    )  # AC-2
                ...
                for line in text.splitlines():
                    ...
                    for opt in names:
                        with self.subTest(command=name, argument=opt):
                            self.assertNotEqual(
                                stripped, opt, "%s -h shows a bare %s" % (name, opt)
                            )  # AC-2
                            self.assertFalse(
                                stripped.startswith(opt + " ")
                                and stripped.split() == [opt, opt.upper().lstrip("-")],
    ...
    def test_the_preflight_base_status_branch_and_clean_branch_texts(self):
        ...
        self.assertTrue(
            _squash(self.sub.choices["preflight"]._option_string_actions["--base"].help)
        )  # AC-2
        self.assertTrue(
            _squash(self.sub.choices["status"]._option_string_actions["--branch"].help)
        )  # AC-2
...
class OneHelperRegistersEveryCommand(unittest.TestCase):
    ...
    def test_the_helper_takes_name_sentence_and_handler(self):
        self.assertTrue(
            callable(getattr(cli, "_command", None)), "cli._command missing"
        )  # AC-3: one helper
...
class CloseStoppedRestoresTheTimestamps(unittest.TestCase):
    ...
    def _close(self, state, message="stopped by user at stage 'review'"):
        ...
        with mock.patch("revali.state.now_iso", return_value=self.NOW):
            with mock.patch(
                "revali.state.write_json_atomic",
                side_effect=PermissionError("state.json is in use"),
            ):
                with redirect_stdout(buf):
...
class ParsingAndDispatchAreUnchanged(unittest.TestCase):
    ...
    def test_flags_and_defaults_parse_as_before(self):
        ...
        self.assertEqual(
            (args.verbose, args.foreground, args.dry_run, args.base), (False, False, False, "")
        )
